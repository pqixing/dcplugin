...
class VersionUtils {
    ...
    static String generatorApplication(Project project) {
        def appTempStr = Templet.applicationTemplet.replace("#packageName", project.packageName)
...
//        println("applicationLike = $applicationLike")
    ...
    static void generatorActivity(Project project) {
        def activityTempStr = Templet.activityTemplet.replace("#packageName", project.packageName).replace("#luanchActivity", project.exts(D.luanchActivity, ""))
        def activityPath = project.file(getUrl(project.exts(D.outDir, project.defaultDir), "java", project.packageName.replace('.', File.separator), "DefaultActivity.java"))
